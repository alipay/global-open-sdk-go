...
Planned for 1.4.0; available on this branch, not yet published as a package. The client accepts
...
The [example](../com/alipay/example/api_key_payment_session/main.go) creates one USD 1.00 CARD checkout session. It rejects
...
```sh
go run ./com/alipay/example/api_key_payment_session
```
...
Optional `WithApiKeyTimeout(30 * time.Second)` sets the total request deadline. Connection establishment and TLS handshake each retain a 15-second limit. Connections are reused; call `Close()` to release idle connections. Environment proxy settings are honored.
